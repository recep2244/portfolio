...
2 0 * * * NEWSLETTER_SEND_CURATION_REMINDER=1 NEWSLETTER_START_CURATION_SERVER=1 NEWSLETTER_SYNC_SUBSCRIBERS=1 \"$ROOT_DIR/newsletter/run_daily.sh\" >> \"$LOG_PATH\" 2>&1 $MARKER"
ENTRY_SEND="CRON_TZ=Europe/London
0 9 * * * NEWSLETTER_AUTO_SEND=1 NEWSLETTER_DELAY_SEND=1 NEWSLETTER_SYNC_SUBSCRIBERS=1 NEWSLETTER_SYNC_ARCHIVE=1 NEWSLETTER_SYNC_PUSH=1 \"$ROOT_DIR/newsletter/run_daily.sh\" >> \"$LOG_PATH\" 2>&1 $MARKER"
...
ENTRY_WEEKLY_REPORT="CRON_TZ=Europe/London
0 8 * * 1 cd \"$ROOT_DIR/newsletter\" && /home/recep/anaconda3/bin/python3 send_weekly_report.py >> \"$LOG_PATH\" 2>&1 $MARKER"

CURRENT_CRON="$(crontab -l 2>/dev/null || true)"
FILTERED_CRON="$(printf "%s\n" "$CURRENT_CRON" | grep -v "$MARKER" || true)"

printf "%s\n%s\n%s\n%s\n%s\n%s\n%s\n" "$FILTERED_CRON" "$ENTRY_CURATION" "$ENTRY_SEND" "$ENTRY_APPROVAL_REMINDER" "$ENTRY_WEEKLY_CURATION" "$ENTRY_DAILY_SYNC" "$ENTRY_WEEKLY_REPORT" | crontab -
